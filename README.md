...
| `/download-report/{run_id}` | GET | Serves `.md` report as file download for email attachment |

#### 📊 Automated Reporting
- Every run saves both `.json` and `.md` reports to `automation/reports/`
- Every execution logged to `automation/logs/api_runs.log` (status, duration, dataset, report path)
- `rag_used: true/false` field in every response — confirms whether RAG was active
...
- If documents exist in `knowledge/` and index is built → `use_rag=True` passed to agents automatically
...
FastAPI /analyze-email              ← separates CSV from documents automatically
    ...
    │  CSV → run_v2_pipeline() → RAG auto-detected
    │
    ▼
...
Logged                              automation/logs/api_runs.log
    │
    ▼
n8n Check Success (IF node)
    │
    ├── SUCCESS → GET /download-report/{run_id} → Email .md to original sender
    │
...
│   │   └── test_setup.py       # Pipeline health check script
│   ├── workflows/
│   │   ├── m1_scheduled_pipeline.json
│   │   ├── m2_schedule_api.json
│   │   └── m3_gmail_listener.json
│   ├── reports/                # Auto-generated .json + .md reports (not committed)
│   └── logs/
│       ├── api_runs.log        # Execution log — JSON lines (not committed)
...
### ⬜ V4 — MLOps Layer
Train ML models on uploaded datasets. Track experiments with MLflow. Version and deploy models via FastAPI. Monitor model drift over time. Docker + cloud deployment + authentication.
